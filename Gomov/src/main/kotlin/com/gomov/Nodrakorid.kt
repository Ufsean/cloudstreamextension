...
class Nodrakorid : Gomov() {
                ...
                is TvSeriesLoadResponse -> {
                    ...
                    this.episodes = when {
                        ...
                        doc.select("div#download").isEmpty() -> {
                            doc.select("div.entry-content p:contains(Episode)").distinctBy { it.text() }
                                .mapNotNull { eps ->
                                    val endSibling =
                                    ...
                                    val siblings =
                                        eps.nextElementSiblingsUntil(endSibling).map { ele ->
                            ...
                            doc.select("div#download h3.title-download").mapNotNull { eps ->
                                val siblings =
                                    eps.nextElementSibling()?.select("li")?.map { ele ->
    ...
    override suspend fun loadLinks(
        ...
        return if (data.startsWith("[")) {
            tryParseJson<ArrayList<LinkData>>(data)?.apmap { linkData ->
                linkData.second.apmap { link ->
                    loadExtractor(
    ...
    private fun String.toEpisode(): Int? {
        return Regex("(?i)Episode\\s?([0-9]+)").find(this)?.groupValues?.getOrNull(1)
            ?.toIntOrNull()
    }
