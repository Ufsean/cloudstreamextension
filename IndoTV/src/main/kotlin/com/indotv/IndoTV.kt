...
class IndoTV : MainAPI() {
    ...
    override suspend fun loadLinks(
        ...
        if (loadData.url.contains("mpd")) {
            callback.invoke(
                    newDrmExtractorLink(this.name, this.name, loadData.url, INFER_TYPE, uuid = java.util.UUID.randomUUID()) {
            ...
            callback.invoke(
                    newExtractorLink(this.name, this.name, loadData.url, INFER_TYPE)
            )
        } else {
            callback.invoke(
                    newExtractorLink(this.name, loadData.title, loadData.url, INFER_TYPE)
            )
        }
        return true
    }
